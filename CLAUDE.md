...
python manage.py ensure_cookie_from_env  # write LINKEDIN_STORAGE_STATE_B64 to cookie file (Railway)
```
...
- Root URL `/` → `linkedin.views.landing_page` renders a landing page with links to CRM (`/crm/`), Django Admin (`/admin/`), and LinkedIn Login (`/linkedin-login/`).
...
- **`browser/login.py`** — Playwright browser setup and LinkedIn login. `launch_browser()` creates a fresh browser instance. `start_browser_session(session, handle)` loads saved cookies or performs fresh login; if cookie file exists but feed redirect fails (expired/invalid), falls back to `playwright_login` and overwrites the cookie file. `playwright_login(session)` performs email/password login with human-like typing. When LinkedIn shows a checkpoint (verification) page, `_wait_for_feed_after_submit()` waits up to 5 minutes for the user to complete it via VNC. (Moved from `navigation/login.py`; `navigation/login.py` remains as a backwards-compat shim.)
- **`browser/nav.py`** — Browser navigation utilities. `goto_page(session, action, expected_url_pattern)` navigates and checks the URL **path** (not full URL) for the pattern — avoids false pass when login redirect has `session_redirect=.../feed/...` in query. Auto-discovers `/in/` URLs via `_extract_in_urls()`. `_discover_and_enrich(session, urls)` auto-enriches discovered profiles (Voyager API + create Lead + embed), rate-limited by `enrich_min_interval` (1s). `human_type(locator, text)` types with random per-keystroke delay (50-200ms). `find_top_card(session)` finds profile card with fallback selectors (`TOP_CARD_SELECTORS`). `find_first_visible(page, selectors)` returns first visible locator. `dump_page_html(session, profile)` saves HTML to fixtures. `random_sleep()` provides human-like wait timing. (Moved from `navigation/utils.py`; `navigation/utils.py` remains as a backwards-compat shim.)
...
- **`django_settings.py`** — Django settings importing DjangoCRM's default settings. SQLite DB at `assets/data/crm.db`. Key settings: `SECRET_KEY` = hardcoded dev key, `DEBUG = True`, `ALLOWED_HOSTS = ["*"]`, `CSRF_TRUSTED_ORIGINS` (set from `RAILWAY_PUBLIC_DOMAIN` when on Railway to fix 403 on login POST), `SITE_ID = 1`, `SITE_TITLE = "OpenOutreach CRM"`, `ADMIN_HEADER = "OpenOutreach Admin"`, `MEDIA_ROOT = DATA_DIR / "media"` (Path, not str), `DJANGO_ALLOW_ASYNC_UNSAFE = "true"`. INSTALLED_APPS includes all DjangoCRM apps + `linkedin`.
...
- **`cookie_utils.py`** — Shared cookie conversion for LinkedIn session. `derive_handle(email)` returns handle from email (part before `@`, lowercased, dots/plus → underscores). `convert_cookies_to_playwright(cookies_json)` converts browser-extension JSON array to Playwright storage-state format. Used by `ensure_cookie_from_env` and the LinkedIn Login view.
- **`views.py`** — `landing_page` renders root landing. `linkedin_login` (staff-only, `/linkedin-login/`) provides two modes: (1) paste cookies (JSON from browser extension) → save to `COOKIES_DIR/<handle>.json`; (2) email/password → run Playwright login, save session. Optional "Restart daemon to apply" writes `RESTART_REQUESTED_PATH`.
